...
let getAllGoogleRestaurant = async (req, res) => {
  ...
  try {
    let restaurants = await GoogleRestaurant.findAndCountAll(obj);
...
let googleRestaurantPlaces = async (req, res) => {
  try {
    const { location, radius, type } = req.body;
    const nearbyURL = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${location}&radius=${radius}&type=${type}&key=${process.env.GOOGLE_API_KEY}`;
    const nearbyRes = await axios.get(nearbyURL);
    const restaurants = nearbyRes.data.results;

    const detailedRestaurants = [];

    for (const r of restaurants) {
      const detailsURL = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${r.place_id}&fields=address_components,name,formatted_address,formatted_phone_number,geometry,opening_hours,photos,place_id,rating,reference,reviews,user_ratings_total,url,utc_offset,vicinity,website&key=${process.env.GOOGLE_API_KEY}`;
      ...
      const restaurant = {
        ...
        street_number: getComponent(data.address_components, "street_number"),
        street_name: getComponent(data.address_components, "route"),
        district: getComponent(
          data.address_components,
          "administrative_area_level_2"
        ),
        city: getComponent(data.address_components, "locality"),
        state: getComponent(
          data.address_components,
...
let googleSingleRestaurantByPlaceID = async (req, res) => {
  try {
    ...
    if (place_id) {
      // const detailsURL = `https://maps.googleapis.com/maps/api/place/details/json?place_id=${place_id}&fields=name,formatted_address,formatted_phone_number,geometry,opening_hours,photos,place_id,rating,reference,reviews,user_ratings_total,url,utc_offset,vicinity,website&key=${process.env.GOOGLE_API_KEY}`;
      const sampleData = {
        html_attributions: [],
        result: {
          address_components: [
...
async function saveRestaurantToDB(restaurant) {
  console.log("Saving to DB:", restaurant.name);
  const transaction = await sequelize.transaction(); // Start a transaction

  try {
    const newGoogleRestaurant = await GoogleRestaurant.create(restaurant, {
      transaction,
    });

    if (restaurant.photos && restaurant.photos.length > 0) {
      for (const data of restaurant.photos) {
        await GoogleResPhoto.create(
          { google_restaurant_id: newGoogleRestaurant.id, ...data },
          { transaction }
        );
      }
    }
    ...
    if (restaurant.reviews && restaurant.reviews.length > 0) {
      for (const data of restaurant.reviews) {
        await GoogleResReview.create(
          { google_restaurant_id: newGoogleRestaurant.id, ...data },
          { transaction }
        );
      }
    ...
    console.log("Saved successfully:", restaurant.name);
  } catch (err) {
